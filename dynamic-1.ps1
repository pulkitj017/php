...
# Function to rename the first column to 'name'
function Rename-FirstColumn {
    param (
        [string]$FilePath,
        [string]$NewColumnName = "name"
    )
    $lines = Get-Content -Path $FilePath
    if ($lines.Count -gt 0) {
        $header = $lines[0] -split '\s+'
        $header[0] = $NewColumnName
        $lines[0] = $header -join " "
        Set-Content -Path $FilePath -Value $lines
    }
}
 
# Rename the first column in each input file
Rename-FirstColumn -FilePath $LicenseFile
Rename-FirstColumn -FilePath $OutdatedFile
Rename-FirstColumn -FilePath $DependencyFile
...
$outputFile = "combined_dependencies-1.txt"
